...
def search_spans(
    ...
    try:
                    ...
                    span_entry.update(
                        {
                            ...
                            if hasattr(attrs, "resource_name")
                            else None,
                            "operation": attrs.operation_name
                            if hasattr(attrs, "operation_name")
                            else None,
        ...
        return ResponseBuilder.success(
            "spans", spans, has_more=has_more, next_cursor=next_cursor, page_size=page_size
        )
...
def get_trace(trace_id: str, auth: DatadogAuth | None = None) -> dict:
    ...
    """
    from datadog_api_client.v2.api.traces_api import TracesApi  # type: ignore[import-not-found]
    ...
    try:
        ...
        response = api_instance.list_traces(filter_query=f"trace_id:{trace_id}", page_limit=1000)
        ...
        if not spans:
            return {"success": False, "error": f"No trace found with ID: {trace_id}"}
        ...
        # Use ResponseBuilder for automatic size limiting
        return ResponseBuilder.success(
            "spans", spans, trace_id=trace_id, root_span=root_span, duration=total_duration
        )
...
def list_services(env: str | None = None, limit: int = 50, auth: DatadogAuth | None = None) -> dict:
    ...
    try:
        ...
        return ResponseBuilder.success("services", services, environment=env)
