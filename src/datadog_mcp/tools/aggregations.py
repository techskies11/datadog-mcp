...
def count_logs(
    ...
    try:
        ...
        if hasattr(response, 'data') and response.data:
            ...
            elif hasattr(response.data, 'attributes') and hasattr(response.data.attributes, 'total'):
                # If no buckets, use total
                count = response.data.attributes.total.count if hasattr(response.data.attributes.total, 'count') else 0
...
def count_unique(
    ...
    try:
        ...
        body = LogsAggregateRequest(
            ...
            compute=[
                LogsCompute(
                    aggregation=LogsAggregationFunction.CARDINALITY,
                    metric=field
                )
            ]
        )
...
def aggregate_logs(
    ...
    to_time: str,
    group_by: str,
    aggregation: Literal["count", "cardinality", "pc75", "pc90", "pc95", "pc99", "sum", "min", "max", "avg"] = "count",
